...
# ---- Robust schema guard (Flask 3.1 safe)
def ensure_schema():
    ...
    ''')
    cur.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('header_title', 'WellAtlas by Henry Suden')")
    conn.commit()
    conn.close()

# Ensure DB exists immediately on import
ensure_schema()
...
def get_setting(key, default=""):
    try:
        return _get_setting_inner(key, default)
    except sqlite3.OperationalError:
        ensure_schema()
        return _get_setting_inner(key, default)


def _get_setting_inner(key, default=""):
    conn = get_db()
    c = conn.cursor()
